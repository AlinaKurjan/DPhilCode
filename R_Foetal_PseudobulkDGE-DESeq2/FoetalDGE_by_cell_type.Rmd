...
wdir = '/mnt/da8aa2c4-0136-465b-87a2-d12a59afec55/akurjan/analysis/rnotebooks/Foetal_PseudobulkDGE/'
setwd(wdir)
...
for (cell_type in cell_types) {
  createSubfolders(wdir, cell_type)
}
```


```{r MAKING DESEQ OBJECTS}
# Create a list to store dds objects for each cell type
dds_list <- list()
dds_lrt_list <- list()
...
# Now you have a list of dds objects for each cell type
# You can access them using dds_list$FGF14_THBS4_Fibroblasts, dds_list$Other_Cell_Type_1, etc.
```
...
for (cell_type in cell_types) {
  ...
  normalised_counts_list[[cell_type]] <- normalised_counts
  write.table(normalised_counts, file=file.path(wdir, cell_type,"normalised_counts.txt"), sep="\t", quote=F, col.names=NA)
...
```{r QC PLOTS}
# Create subfolders and run the code for each cell type
for (cell_type in cell_types) {
  # Load the dds object for the current cell type
  dds <- dds_list[[cell_type]]
  
  rld <- rlog(dds, blind = TRUE) # Regularised-logarithm transformation (Rlog)
  anno <- as.data.frame(colData(rld)[, c("age", "libbatch", "type")])
  ...
  heatmap_file1 <- file.path(wdir, cell_type, "Figures", "QC_sample_distancesimilarity_pheatmap.svg")
  heatmap_file2 <- file.path(wdir, cell_type, "Figures", "QC_sample_distancesimilarity_pheatmap_color.svg")
  pca_scree <- file.path(wdir, cell_type, "Figures", "QC_sample_pca_scree.svg")
  pca_age <- file.path(wdir, cell_type, "Figures", "QC_sample_pca_age.svg")
  pca_libbatch <- file.path(wdir, cell_type, "Figures", "QC_sample_pca_libbatch.svg")
  pca_type <- file.path(wdir, cell_type, "Figures", "QC_sample_pca_type.svg")
 ...
 # PLOT3: Scree plot with proportions of variance explained per PC
  ...
  pca_type_plot <- plotPCA(rld, intgroup = c("type"), ntop = 700)
  print(pca_type_plot)
  dev.off()
...
res_young_list <- list()
res_old_list <- list()
contrast_old <- c("age", "20w", "17w")
contrast_young <- c("age", "17w", "12w")

# Run the DESeq differential expression pipeline for each cell type
for (cell_type in cell_types) {
  # Load the dds object for the current cell type
  dds <- dds_list[[cell_type]]
  ...
  size_factors_plot <- ggplot(plot_table, aes(x = sample, y = size_factors, fill = age)) +
    geom_col() +
    theme(axis.text.x = element_text(angle = 60, hjust = 1))
  ggsave(file.path(wdir, cell_type, "Figures", "size_factors_plot.svg"), size_factors_plot, width = 10, height = 7)

  
  # Plot dispersion estimates
  svg(file.path(wdir, cell_type, "Figures", "dispersion_plot.svg"), width = 10, height = 7)
  plotDispEsts(dds)
  dev.off()
  
  # Get results for contrasts
  res_old <- results(dds, contrast = contrast_old, alpha = 0.05)
  res_young <- results(dds, contrast = contrast_young, alpha = 0.05)
  res_lrt <- results(dds_lrt, alpha = 0.01)
  
  res_lrt_list[[cell_type]] <- res_lrt
  res_young_list[[cell_type]] <- res_young
  res_old_list[[cell_type]] <- res_old
  
  # Save summary metrics to a file
  summary_file <- file.path(wdir, cell_type, "Results", "summary_metrics.txt")
...
for (cell_type in cell_types) {
  # Load the dds object for the current cell type
  dds <- dds_list[[cell_type]]
  res_young <- res_young_list[[cell_type]]
  res_old <- res_old_list[[cell_type]]
  
  res_apeglm_old <- lfcShrink(dds, coef = "age_20w_vs_17w", type="apeglm")
  res_apeglm_young <- lfcShrink(dds, coef = "age_12w_vs_17w", type="apeglm")
  res_ashr_young <- lfcShrink(dds, contrast = c("age", "12w", "17w"), type = "ashr", res = res_young)
  res_ashr_old <- lfcShrink(dds, contrast = c("age", "20w", "17w"), type = "ashr", res = res_old)
  
  res_apeglm_old_list[[cell_type]] <- res_apeglm_old
  res_apeglm_young_list[[cell_type]] <- res_apeglm_young
  res_ashr_old_list[[cell_type]] <- res_ashr_young
  res_ashr_young_list[[cell_type]] <- res_ashr_old
  ...
  svg(file.path(wdir, cell_type, "Figures", "rawMAplot_20wvs17w.tiff"), width = 7, height = 5)
  ...
  svg(file.path(wdir, cell_type, "Figures", "rawMAplot_12wvs17w.tiff"), width = 7, height = 5)
  ...
  svg(file.path(wdir, cell_type, "Figures", "APEGLM_MAplot_12wvs17w.tiff"), width = 7, height = 5)
  plotMA(res_apeglm_young, ylim = c(-5, 5))
  dev.off()
  
  # Save APEGLM_MAplot_1920.svg
  svg(file.path(wdir, cell_type, "Figures", "APEGLM_MAplot_20wvs17w.tiff"), width = 7, height = 5)
  plotMA(res_apeglm_old, ylim = c(-5, 5))
  dev.off()
  
  # Save ASHR_MAplot_1517.svg
  svg(file.path(wdir, cell_type, "Figures", "ASHR_MAplot_12wvs17w.tiff"), width = 7, height = 5)
  plotMA(res_ashr_young, ylim = c(-5, 5))
  dev.off()
  
  # Save ASHR_MAplot_1920.svg
  svg(file.path(wdir, cell_type, "Figures", "ASHR_MAplot_20wvs17w.tiff"), width = 7, height = 5)
...
will use ashr-shrinked values as they appear better.
...
define_and_save_regulated <- function(res_data_df){
  upregulated <- res_data_df %>%
    dplyr::filter(padj < 0.01 & log2FoldChange >= 1)
  downregulated <- res_data_df %>%
    dplyr::filter(padj < 0.01 & log2FoldChange <= -1)
  
  upregulated_file <- file.path(wdir, cell_type, "Results", 
                                paste(strsplit(deparse(substitute(res_data_df)), '_')[[1]][3], "ASHR_upregulated_genes.csv"))
  downregulated_file <- file.path(wdir, cell_type, "Results", 
                                  paste(strsplit(deparse(substitute(res_data_df)), '_')[[1]][3], "ASHR_downregulated_genes.csv"))
  all_genes_file <- file.path(wdir, cell_type, "Results", 
                              paste(strsplit(deparse(substitute(res_data_df)), '_')[[1]][3], "ASHR_all_genes.csv"))
...
create_full_volcano <- function(res_data_df, title, savename){
  ...
  ggsave(file.path(wdir, cell_type, "Figures", savename), volcano, 
         width = 10, height = 8)
}
...
for (cell_type in cell_types) {
  ...
  regulated_young <- define_and_save_regulated(res_ashr_young_df)
  regulated_old <- define_and_save_regulated(res_ashr_old_df)
  
  upregulated_old_ASHR_list[[cell_type]] <- regulated_old$upregulated
  downregulated_old_ASHR_list[[cell_type]] <- regulated_old$downregulated
  upregulated_young_ASHR_list[[cell_type]] <- regulated_young$upregulated
  downregulated_young_ASHR_list[[cell_type]] <- regulated_young$downregulated
  
  create_full_volcano(res_ashr_young_df, '12pcw vs 17pcw Foetal Tendons', "ASHR_Volcano_12wvs17w_legright.tiff")
  create_full_volcano(res_ashr_old_df, '20pcw vs 17pcw Foetal Tendons', "ASHR_Volcano_20vs17w_legright.tiff")
  
  rm(ensid, upregulated, downregulated, res_ashr_young, res_ashr_old,
     res_ashr_young_df, res_ashr_old_df,
     volcano, regulated_young, regulated_old, title, savename)
}
```
...
for (cell_type in cell_types) {
  ...
  heatmap_vst_young <- create_heatmap_vst_all(sig_genes_young, vst_mat, cell_type, target_replicate, "Up in 17w vs 12w")
  heatmap_vst_old <- create_heatmap_vst_all(sig_genes_old, vst_mat, cell_type, target_replicate, "Up in 20w vs 17w")
}
...
for (cell_type in cell_types) {
  ...
  create_custom_volcano(res_ashr_old_df_mat, colors, 
                        '20pcw vs 17pcw Foetal Tendons (ECM Genes)', 
                        "ASHR_Volcano_ECMGenes_20vs17w.tiff")
  create_custom_volcano(res_ashr_young_df_mat, colors, 
                        '12pcw vs 17pcw Foetal Tendons (ECM Genes)', 
                        "ASHR_Volcano_ECMGenes_12vs17w.tiff")
}
...
for (cell_type in cell_types) {
  ...
  sig_genes_ashr_old_list[[cell_type]] <- sig_genes_old
  sig_genes_ashr_young_list[[cell_type]] <- sig_genes_young
  
  #write.csv(sig_genes_old, file = file.path(wdir, cell_type, "Results", "WALD_SignificantGenes_20vs17w_001pval.csv"))
  #write.csv(sig_genes_young, file = file.path(wdir, cell_type, "Results", "WALD_SignificantGenes_12vs17w_001pval.csv"))
  
  #sig_genes_ashr_old_vst <- significant_only_vst_df(sig_genes_old, vst_mat)
  #sig_genes_ashr_young_vst <- significant_only_vst_df(sig_genes_young, vst_mat)
  
  #ego_functionalprof_wald(sig_genes_old, res_old_df, "20vs17w", wdir, cell_type)
  #ego_functionalprof_wald(sig_genes_young, res_young_df, "12vs17w", wdir, cell_type)
  
  gprofiler2_save_gem(sig_genes_old, "20vs17w", wdir, cell_type)
  gprofiler2_save_gem(sig_genes_young, "12vs17w", wdir, cell_type)
}
```
...
for (cell_type in cell_types) {
  ...
  write.table(rankings_old, 
              file.path(wdir, cell_type, "Results","ASHR_WALD_ 20vs17w_GeneRanks.rnk"), 
  ...
  write.table(rankings_young, 
              file.path(wdir, cell_type, "Results","ASHR_WALD_ 12vs17w_GeneRanks.rnk"), 
            col.name = TRUE, sep="\t", row.names = FALSE, quote = FALSE)
}
```
